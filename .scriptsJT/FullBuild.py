...
class ESPBuildManager:
    ...
    def assert_esptool_v5(self) -> None:
        ...
        if major != 5:
            ...
            sys.exit(1)
        print(f"{Fore.GREEN}Using {ver_text}{Style.RESET_ALL}")

    def esptool_cmd(self, *args: str) -> List[str]:
        return self.esptool_base_cmd + list(args)

    def get_platformio_base_cmd(self) -> List[str]:
        """Resolve a working PlatformIO command for this Python environment."""
        for cmd in ("platformio", "pio"):
            resolved = shutil.which(cmd)
            if resolved:
                return [resolved]
        # Fallback for environments where scripts are not on PATH.
        return [sys.executable, "-m", "platformio"]

    def run_platformio(self, args: List[str], check: bool = True):
        """Run PlatformIO using the resolved command."""
        return subprocess.run(self.get_platformio_base_cmd() + args, check=check)
    # -------------------------------------------

    def check_dependencies(self) -> bool:
        """Check that all required tools are installed"""
        print("Checking dependencies...")
        python_ok = shutil.which("python") or shutil.which("py") or shutil.which("python3")
        pio_ok = bool(shutil.which("platformio") or shutil.which("pio"))

        if not pio_ok:
            # Validate module fallback path too.
            try:
    ...
    def get_serial_port_with_retries(self, preferred: Optional[str] = None,
                                     retries: Optional[int] = None,
                                     delay: Optional[int] = None) -> str:
        """Resolve a usable serial port with retry/backoff."""
        flash_cfg = self.config.get('flash', default={}) if isinstance(self.config.get('flash', {}), dict) else {}
        max_retries = retries if retries is not None else flash_cfg.get('retry_attempts', 3)
        retry_delay = delay if delay is not None else flash_cfg.get('retry_delay', 5)
        ...
        while attempt <= max_retries:
            try:
                # If a preferred port was provided, verify it is openable
                if preferred:
                    print(f"{Fore.CYAN}🔌 Using specified port {preferred}...{Style.RESET_ALL}")
                    try:
                        with serial.Serial(preferred, 115200, timeout=1) as _:
                            pass
                    except Exception as e:
                        raise serial.SerialException(f"Specified port not ready: {e}")
                    return preferred

                # Otherwise auto-detect
                self.kill_serial_monitors()
                port = self.find_serial_port()  # may raise
                # sanity open test
                try:
                    with serial.Serial(port, 115200, timeout=1) as _:
                        pass
                except Exception as e:
                    raise serial.SerialException(f"Detected port not ready: {e}")
                ...
                if attempt <= max_retries:
                    print(f"{Fore.YELLOW}⚠ Could not connect to device (attempt {attempt}/{max_retries}). "
                          f"Retrying in {retry_delay} seconds...{Style.RESET_ALL}")
                    time.sleep(retry_delay)
                else:
                    break

        print(f"{Fore.RED}✘ Unable to get a usable serial port after {max_retries} retries: {last_err}{Style.RESET_ALL}")
        sys.exit(1)
    ...
    def backup_firmware(self, serial_port: str, backup_dir: Optional[Path] = None) -> Path:
        ...
        print(f"Backing up firmware to {backup_file}")
    ...
    def validate_firmware(self, serial_port: str, timeout: Optional[int] = None) -> bool:
        ...
        print(f"Validating firmware on {serial_port}")
    ...
    def erase_flash(self, serial_port: str,
                    max_retries: Optional[int] = None,
                    retry_delay: Optional[int] = None) -> None:
        """Erase the entire flash of the ESP32 (esptool v5) with retries/backoff."""
        ...
        while attempt <= max_retries:
            ...
            # Try to force bootloader before each attempt
            ...
            try:
                ...
                attempt += 1
                print(f"{Fore.RED}✘ Erase attempt {attempt} failed with return code {e.returncode}.{Style.RESET_ALL}")
                if attempt <= max_retries:
                    print(f"{Fore.YELLOW}🔁 Retrying erase in {retry_delay} seconds...{Style.RESET_ALL}")
                    time.sleep(retry_delay)
                else:
                    print(f"{Fore.RED}💥 All erase attempts failed. Aborting.{Style.RESET_ALL}")
                    sys.exit(1)
    ...
    def launch_serial_monitor(self, port: str, baud: int = 115200) -> None:
        ...
        try:
            self.run_platformio(["device", "monitor", "--port", port, "--baud", str(baud)], check=False)
        except Exception as e:
    ...
    def clean_environment(self, env_name: str) -> None:
        ...
        self.run_platformio(["run", "-t", "clean", "-e", env_name], check=True)
    ...
    def process_environment(self, env_name: str, serial_port: str, profile: Optional[BuildProfile] = None) -> None:
        ...
        config = BuildConfig(
            ...
            fs_offset=fs_offset,
            fs_size=fs_size,
            baud_rate=profile.flash_config.get('baud_rate', 921600) if profile else 921600
        )
        ...
        # Erase if requested (single location controlling erase policy)
        if not profile or profile.erase_before_flash:
            self.erase_flash(serial_port)  # now with retries/backoff
        else:
            # Still ensure bootloader before flashing if we didn't just erase
            self.enter_bootloader(serial_port)
        
        # Flash (no second erase)
    ...
    def run_action(self, action: str) -> None:
        ...
        if action in ['flash', 'full', 'monitor', 'backup']:
            preferred = self.args.port if self.args and self.args.port else None
            serial_port = self.get_serial_port_with_retries(preferred=preferred)
