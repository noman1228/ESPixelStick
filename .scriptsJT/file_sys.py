...
def get_platformio_cmd():
    # Prefer direct CLI binaries if available, then fall back to Python module execution.
...
def build_littlefs_image(env_name):
    pio_cmd = get_platformio_cmd()
    pretty_cmd = " ".join(pio_cmd + ["run", "-t", "buildfs", "-e", env_name])
    print(f"{Fore.CYAN}🔧 Running: '{pretty_cmd}'...{Style.RESET_ALL}")
    subprocess.run(pio_cmd + ["run", "-t", "buildfs", "-e", env_name], check=True)
