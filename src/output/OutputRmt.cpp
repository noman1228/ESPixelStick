...
//------------------------------------------------------------------------------
// Background task: feeds frames and waits on ISR notifications
//------------------------------------------------------------------------------
    ...
    while (1)
    {
        ...
        for (c_OutputRmt* pRmt : rmt_isr_ThisPtrs)
        {
            if (nullptr != pRmt)
            {
                ...
                {
                    // Wait up to 100ms for end-of-frame notify
...
void c_OutputRmt::Begin(OutputRmtConfig_t config, c_OutputCommon* _pParent)
{
    do
    {
        ...
        NumRmtSlotsPerIntensityValue = OutputRmtConfig.IntensityDataWidth + ((OutputRmtConfig.SendInterIntensityBits) ? 1u : 0u);
        if (OutputRmtConfig_t::DataDirection_t::MSB2LSB == OutputRmtConfig.DataDirection)
        {
            TxIntensityDataStartingMask = 1u << (OutputRmtConfig.IntensityDataWidth - 1u);
        }
        else
        {
            TxIntensityDataStartingMask = 1u;
        }

        // --- RMT HW configuration ---
        ...
        RmtConfig.mem_block_num = rmt_reserve_memsize_t::RMT_MEM_64; // keep as-is for your layout

        RmtConfig.tx_config.carrier_freq_hz    = uint32_t(10); // ignored when carrier_en=false
        ...
        RMT.apb_conf.mem_tx_wrap_en = 1;  // wrap enable

        // Reset and own TX memory
...
//------------------------------------------------------------------------------
// Bit Translation Table Helpers
//------------------------------------------------------------------------------
void c_OutputRmt::UpdateBitXlatTable(const CitrdsArray_t* CitrdsArray)
{
    // FIX: check the parameter, not OutputRmtConfig.CitrdsArray
    if (nullptr != CitrdsArray)
    {
...
bool c_OutputRmt::ValidateBitXlatTable(const CitrdsArray_t* CitrdsArray)
{
    ...
    if (nullptr != CitrdsArray)
    {
        ...
        while (CurrentTranslation->Id != RmtDataBitIdType_t::RMT_LIST_END)
        {
            ...
            {
                logcon(String(CN_stars) + F("ERROR: incorrect bit translation detected. Chan: ") + String(OutputRmtConfig.RmtChannelId) +
        ...
        // If we get here, it at least iterated successfully; set as true.
        Response = true;
    }
    else
    {
...
void c_OutputRmt::GetStatus(ArduinoJson::JsonObject& jsonStatus)
{
    ...
    // FIX: keep separate snapshots for ena and st
    uint32_t ena = RMT.int_ena.val;
    uint32_t sts = RMT.int_st.val;
    debugStatus["Raw int_ena"]                  = "0x" + String(ena, HEX);
    debugStatus["Raw int_st"]                   = "0x" + String(sts, HEX);
    ...
    debugStatus["TX END int_ena"]               = "0x" + String(ena & (RMT_INT_TX_END_BIT), HEX);
    debugStatus["TX END int_st"]                = "0x" + String(sts & (RMT_INT_TX_END_BIT), HEX);
    debugStatus["TX THRSH int_ena"]             = "0x" + String(ena & (RMT_INT_THR_EVNT_BIT), HEX);
    debugStatus["TX THRSH int_st"]              = "0x" + String(sts & (RMT_INT_THR_EVNT_BIT), HEX);
...
#ifdef USE_RMT_DEBUG_COUNTERS
        if (IntensityValue > 200) { ++RmtWhiteDetected; }
#endif

        uint32_t bitmask = TxIntensityDataStartingMask;
        for (uint32_t BitCount = OutputRmtConfig.IntensityDataWidth; BitCount > 0; --BitCount)
        {
            // Evaluate current bit BEFORE modifying bitmask (FIX for counters consistency)
            const bool bit_set = (IntensityValue & bitmask) != 0;
            RMT_DEBUG_COUNTER(IntensityBitsSent++);
            ISR_WriteToBuffer(bit_set ? OneBitValue : ZeroBitValue);

#ifdef USE_RMT_DEBUG_COUNTERS
            if (bit_set) { BitTypeCounters[int(RmtDataBitIdType_t::RMT_DATA_BIT_ONE_ID)]++; }
            else         { BitTypeCounters[int(RmtDataBitIdType_t::RMT_DATA_BIT_ZERO_ID)]++; }
#endif
            if (OutputRmtConfig_t::DataDirection_t::MSB2LSB == OutputRmtConfig.DataDirection)
            {
                bitmask >>= 1;
...
void IRAM_ATTR c_OutputRmt::ISR_Handler(uint32_t isrFlags)
{
    ...
    else if (isrFlags & RMT_INT_THR_EVNT_BIT)
    {
        ...
        if (NumUsedEntriesInSendBuffer)
        {
            ...
            if (!ThereIsDataToSend && (NumUsedEntriesInSendBuffer == 0))
            {
    ...
    else
    {
        ...
        if (isrFlags & RMT_INT_ERROR_BIT) { ErrorIsr++; }
        if (isrFlags & RMT_INT_RX_END_BIT) { RxIsr++; }
    }
#endif
}

//------------------------------------------------------------------------------
// Helpers used by ISR / frame start
//------------------------------------------------------------------------------
...
{
    RMT.conf_ch[OutputRmtConfig.RmtChannelId].conf1.mem_rd_rst = 1;  // reset read ptr
...
{
    uint32_t NumEntriesToTransfer = std::min(NumUsedEntriesInSendBuffer, MaxNumEntriesToTransfer); // FIX: std::min
    ...
    // Clear the next slot to hard-stop the DMA reader when we run out.
...
bool c_OutputRmt::StartNewFrame()
{
    ...
    do
    {
        ...
        // NOTE: InterrupsAreEnabled appears to be a macro in your tree; left as-is.
